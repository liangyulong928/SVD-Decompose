...
def plt_sketch(dataset,U,T):
    ...
    for i in range(9):
        ...
        plt.savefig(f"./data-sketch-{dataset}/taskRelaiton-u{i+1}.png",dpi=300) 
    ...
    for i in range(9):
        ...
        ax.scatter(x, y, s=0.5, c='k', marker='.')
        plt.savefig(f"./data-sketch-{dataset}/taskRelaiton-v{i+1}.png",dpi=300) 
...
def dataset1():
    with open('yelp.edgelist') as file:
        ...
        for line in file.readlines():
            ...
            row.append(floatLine[0])
            col.append(floatLine[1])
            data.append(floatLine[-1])
